...
export default function ServiceLineModal({ initial, categoryId, categoryName, onClose, onSaved }) {
  const isEdit = !!initial;
  const [form, setForm] = useState({
    name: initial?.name ?? "", description: initial?.description ?? "",
    image_url: initial?.image_url ?? "", display_order: initial?.display_order ?? 0,
    is_active: initial?.is_active ?? true, category_id: initial?.category_id ?? categoryId,
  });
  ...
  const handleSave = async (e) => {
    ...
    try {
      const payload = { ...form, display_order: Number(form.display_order), category_id: Number(form.category_id) };
      const res = isEdit
  ...
  return (
    ...
    >
      <Field label="Name" required>
        <input required className={inp} value={form.name} onChange={(e) => set("name", e.target.value)} placeholder="e.g. Shirt, Blazer, Trouser" />
      </Field>
      <Field label="Description"><textarea rows={2} className={tinp} value={form.description} onChange={(e) => set("description", e.target.value)} placeholder="Brief description shown on the customer app" /></Field>
